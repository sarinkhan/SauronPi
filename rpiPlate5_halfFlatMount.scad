...
module openBasePlate3(chassX,chassY,beamsThick,nbBeamsX=3,nbBeamsY=2,thickness01=5)
{
	xShift=chassX/(nbBeamsX+1);
	for (i =[0:nbBeamsX+1])
	{
		translate([xShift*i,0,0])
			cube([beamsThick,chassY,thickness01]);
	}

	yShift=chassY/(nbBeamsY+1);
	for (i =[0:nbBeamsY+1])
	{
		translate([0,yShift*i,0])
			cube([chassX+beamsThick,beamsThick,thickness01]);
	}
}
